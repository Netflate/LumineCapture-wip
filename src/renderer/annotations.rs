...
pub fn draw_annotation(canvas: &mut Pixmap, ann: &Annotation, selected: bool) {
    match &ann.shape {
...
fn draw_annotation_handles(canvas: &mut Pixmap, bbox: &Rect) {
    let mut paint = Paint::default();
    ...
    if let Some(path) = pb.finish() { canvas.stroke_path(&path, &paint, &stroke, Transform::identity(), None); }
