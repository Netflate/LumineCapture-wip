...
// all of this temporarily poor implemented generated by ai stuff
        ...
        match event {
            wl_keyboard::Event::Key { key, state: key_state, .. } => {
                ...
                if pressed {
                    ...
                    } else if key == 46 && state.ctrl_held {
                        state.events.push_back(OverlayEvent::SaveToClipboard);
                    } else if key == 44 && state.ctrl_held {
                        if state.shift_held {
                            state.events.push_back(OverlayEvent::Redo);
                        } else {
                            state.events.push_back(OverlayEvent::Undo);
                        }
                    } else if key == 21 && state.ctrl_held {
                        state.events.push_back(OverlayEvent::Redo);
                    } else {
                        match key {
                            30  => {
                                if state.ctrl_held {
                                    state.events.push_back(OverlayEvent::KeyPress(SpecialKey::KeyA));
                                } else if let Some(c) = keycode_to_char(key, state.shift_held) {
                                    state.events.push_back(OverlayEvent::TextInput(c));
                                }
                            }
                            14  => state.events.push_back(OverlayEvent::KeyPress(SpecialKey::Backspace)),
                            111 => state.events.push_back(OverlayEvent::KeyPress(SpecialKey::Delete)),
                            28  => state.events.push_back(OverlayEvent::KeyPress(SpecialKey::Enter)),
                            105 => state.events.push_back(OverlayEvent::KeyPress(SpecialKey::Left)),
                            106 => state.events.push_back(OverlayEvent::KeyPress(SpecialKey::Right)),
                            103 => state.events.push_back(OverlayEvent::KeyPress(SpecialKey::Up)),
                            108 => state.events.push_back(OverlayEvent::KeyPress(SpecialKey::Down)),
                            102 => state.events.push_back(OverlayEvent::KeyPress(SpecialKey::Home)),
                            107 => state.events.push_back(OverlayEvent::KeyPress(SpecialKey::End)),
                            _ => {
                                if !state.ctrl_held {
                                    if let Some(c) = keycode_to_char(key, state.shift_held) {
                                        state.events.push_back(OverlayEvent::TextInput(c));
                                    }
                                }
                            }
            ...
            wl_keyboard::Event::Modifiers { mods_depressed, .. } => {
                ...
                state.events.push_back(OverlayEvent::ModifiersChanged {
